...
//Added by Richie
	...
	function get_dependents_list($id, $cur_tab_id, $rel_tab_id, $relationId) {
		...
		if ($numOfFields > 0) {
			...
			$query .= " INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = $other->table_name.$other->table_index";
			$query .= $more_relation;
/**PVTPATCHER-C029651016E27AE26170FA7B0B9D9C44-START-advcf**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:23 **/
if (strpos(strtolower($dependentColumn), 'rf_pcf_mrf') !== false) {
            $query .= " INNER JOIN $this->table_name AS $this->table_name$this->moduleName ";
            $query .= " Right JOIN vtiger_multireference_rel ON
            $this->table_name$this->moduleName.$this->table_index = vtiger_multireference_rel.crmid
            AND $other->table_name.$other->table_index = vtiger_multireference_rel.targetid";
                    } else {
...
/** REPLACED-C029651016E27AE26170FA7B0B9D9C44// $query .= " INNER JOIN $this->table_name AS $this->table_name$this->moduleName ON $this->table_name$this->moduleName.$this->table_index = $dependentTableName.$dependentColumn";**/
/**PVTPATCHER-C029651016E27AE26170FA7B0B9D9C44-FINISH**/
			$query .= " LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid";
			$query .= " LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid";
