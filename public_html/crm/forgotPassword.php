...
if (isset($_REQUEST['username']) && isset($_REQUEST['emailId'])) {
    ...
    if ($email && vtlib_purify($_REQUEST['emailId']) == $email) {
        $time = time();
        $options = array(
        ...
        */
        $subject = vtranslate('Request : ForgotPassword','ParsVT');
        $content = sprintf(vtranslate('LBL_FORGETPASSWORD_EMAIL','ParsVT'),$username,$trackURL,date("Y-m-d H:i:s"),$HELPDESK_SUPPORT_NAME);
