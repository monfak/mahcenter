	...
	public function getFromClause() {
		...
		foreach ($fieldList as $key => $fieldName) {
			...
			if (empty($referenceParentFieldName)) {
				...
				if ($fieldType == 'owner') {
                    			$tableList['vtiger_users'.$fieldName] = 'vtiger_users';
                    			$tableList['vtiger_groups'.$fieldName] = 'vtiger_groups';
                    			$tableJoinMapping['vtiger_users'.$fieldName] = 'LEFT JOIN';
                    			$tableJoinMapping['vtiger_groups'.$fieldName] = 'LEFT JOIN';
				}
		...
		foreach ($this->whereFields as $key=>$fieldName) {
			if (empty($fieldName))
				continue;
...
//            		/**
//             		* #50996 - If we have owner & owner group fields in a module then there is a chance of taking
//             		* owner group as owner field. But we need to take assigned_user_id as owner
//             		*/
//            		foreach($ownerFields as $ownerFieldName){
//             			$field = $moduleFields[$ownerFieldName];
//                		if($field->getUIType() == 53 && !$field->isCustomField()){
//                    			$ownerField = $ownerFieldName;
//                    			break;
//                		}
//            		}
//        	} else {
...
//        	}
		...
		foreach ($tableList as $key => $tableName) {
			if ($tableName ==  'vtiger_users' && !empty($fieldName =substr( $key, 12 )) ) {
				$field = $moduleFields[$fieldName];
                		$tableNameAS = $tableName. '_' .$fieldName;
				$sql .= " $tableJoinMapping[$key] $tableName AS  $tableNameAS ON ".$field->getTableName()."." .
						$field->getColumnName()." = $tableNameAS.id";
			} elseif ($tableName  ==  'vtiger_groups'  && !empty($fieldName =substr( $key, 13 ))) {
               			 $field = $moduleFields[$fieldName];
                		 $tableNameAS = $tableName. '_' .$fieldName;
				 $sql .= " $tableJoinMapping[$key] $tableName AS $tableNameAS ON ".$field->getTableName()."." .
						$field->getColumnName()." = $tableNameAS.groupid";
		...
		if ($orderByFieldModel && $orderByFieldModel->getFieldDataType() == 'reference') {
			...
			if ($parentReferenceField) {
				...
				$orderByColumn = "COALESCE(CONCAT($userTableName.first_name,$userTableName.last_name),$groupTableName.groupname)";
                		...
                		$ownerTableName = '_'.$fieldName;
				$orderByColumn = "COALESCE(CONCAT(vtiger_users$ownerTableName.first_name,vtiger_users$ownerTableName.last_name),vtiger_groups$ownerTableName.groupname)";
			}
