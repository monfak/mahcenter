...
class QueryGenerator {
	...
	public function __construct($module, $user) {
		...
		$this->ownerFields = $this->getOwnerFields();
		$this->columns = null;
...
/**PVTPATCHER-F708494B3CA7DF0A8E0A9739DEACF91C-FINISH**/
		...
		foreach ($valueArray as $value) {
			...
			if((strtolower(trim($value)) == 'null') ||
					...
					($operator == 'e' || $operator == 'n') && ($field->getFieldDataType() != 'owner' && $field->getFieldDataType() != 'ownergroup')) {
				if($operator == 'e'){
					$sql[] = "IS NULL";
					$sql[] = "= ''";
					continue;
			...
			} else if ($field->getFieldDataType() === 'multicurrency') {
			...
			switch($operator) {
				...
				case 'ew': $sqlOperator = "LIKE";
					$value = "%$value";
					break;
				case 'c': if($field->getFieldName() == 'currency_id' && is_numeric($value)) {
					$sqlOperator = "=";
					break;
				}
					$sqlOperator = "LIKE";
