...
/**PVTPATCHER-75FC4FD0B1AE3147FD0DA777B4D62ECB-START-ws**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:38 **/
$qty_i = isset($_REQUEST['qty'.$i])?$_REQUEST['qty'.$i]:(isset($_REQUEST['quantity'.$i])?$_REQUEST['quantity'.$i]:0);
        $qty = vtlib_purify($qty_i);
        $listprice_i = isset($_REQUEST['listprice'.$i])?$_REQUEST['listprice'.$i]:(isset($_REQUEST['listPrice'.$i])?$_REQUEST['listPrice'.$i]:0);
        $listprice = vtlib_purify($listprice_i);
/**PVTPATCHER-75FC4FD0B1AE3147FD0DA777B4D62ECB-FINISH**/
