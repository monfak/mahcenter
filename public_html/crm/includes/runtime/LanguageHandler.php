...
class Vtiger_Language_Handler {
	...
	public static function getTranslatedString($key, $module = '', $currentLanguage = '') {
        ...
        if (strlen($currentLanguage) != 5 || strpos($currentLanguage, '_') !== true ) {
            $currentLanguage = self::getLanguage();
        }
	...
	public static function getLanguage() {
		...
		$language = '';
        if (!empty($userModel) && ($userModel->has('language') || !empty($userModel->get('language')))) {
...
function vtranslate($key, $moduleName = '') {
	...
	if(parsvt_php7_count($unformattedArgs) > 2){
    ...
    if(strpos($formattedString, '%s') !== false && is_array($args) && !empty($args)) {
...
function parsvt_php7_count($value) {
