...
{strip}
    ...
    <div class="clearfix">
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
            <h3 style="margin-top: 0px;">{vtranslate('API Access Logs', $QUALIFIED_MODULE)}</h3>
        </div>
    </div>
    <hr>
		...
		<div id="listview-actions" class="listview-actions-container">
            <div class="listSearchActionContainer">
                ...
                <div class="col-md-6 pull-left">
                    ...
                    <button class="btn btn-default btn-inverse hide deleteLogs" onclick="javascript:Settings_ParsVT_APIAccessList_Js.deleteSelectedAPILogs();"><strong><i class="fa fa-trash"></i>&nbsp;{'LBL_DELETE'|@vtranslate:$MODULE}</strong></button>&nbsp;&nbsp;&nbsp;
                    <button class="btn btn-danger hide cleanLogs" onclick="javascript:Settings_ParsVT_APIAccessList_Js.cleanAPILogs();"><strong><i class="fa fa-trash"></i>&nbsp;{'Delete all records'|@vtranslate:$MODULE}</strong></button>&nbsp;&nbsp;&nbsp;
                </div>
            </div>
                        ...
                        <table id="listview-table"  class="table listview-table">
                                ...
                                <tr class="searchRow listViewSearchContainer">
                                    <th class="inline-search-btn">
                                        <div class="table-actions" style="width : auto !important; display: flex">
                                            <button class="searchBtn btn btn-primary"  data-trigger="apilogs_listSearch">
                                                <i class="fa fa-search"></i> {vtranslate('LBL_SEARCH', $MODULE)}
                                            </button>
                                            <button class="searchAndClearButton btn btn-danger {if !$SEARCH_DETAILS ||count($SEARCH_DETAILS) eq 0}hide{/if}" style="width: auto !important;" data-trigger="apilogs_ClearSearch"><i class="fa fa-close"></i></button>
                                        </div>      
                                    </th>
                                    {foreach key=LISTVIEW_HEADER_NAME item=LISTVIEW_FIELD_INFO from=$FIELDS_INFO}
                                        <th>
                                            {if $LISTVIEW_HEADER_NAME eq 'accesstime'}
                                                <input type="text" name="accesstime" placeholder="{vtranslate('LBL_SELECT_DATE_RANGE',$MODULE)}" class="listSearchContributor inputElement dateField" data-date-format="{$CURRENT_USER_MODEL->get('date_format')}" data-calendar-type="range" value="{$SEARCH_DETAILS[$LISTVIEW_HEADER_NAME]}"  data-field-type="datetime" id="apilogs_{$LISTVIEW_HEADER_NAME}"/>
                                            {else if $LISTVIEW_FIELD_INFO['type'] eq 'picklist'}
                                                {assign var=PICKLIST_VALUES value=ParsVT_APIAccessList_View::getSearchableValues($LISTVIEW_HEADER_NAME)}
                                                <select class="select2 col-lg-11 col-md-11 col-sm-11 listSearchContributor" name="{$LISTVIEW_HEADER_NAME}"  id="apilogs_{$LISTVIEW_HEADER_NAME}" >
                                            ...
                                            {else}
                                                <input type="text" name="{$LISTVIEW_HEADER_NAME}" class="listSearchContributor inputElement" value="{$SEARCH_DETAILS[$LISTVIEW_HEADER_NAME]}" id="apilogs_{$LISTVIEW_HEADER_NAME}"/>
                                            {/if}
                                        </th>
                                    {/foreach}
                                </tr>
                            ...
                            {foreach key=RECORD_ID item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES}
                                <tr class="listViewEntries" data-id="{$RECORD_ID}">
                                    <td class="listViewRecordActions"> {$LISTVIEW_ENTRY['id']}</td>
