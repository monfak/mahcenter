		...
		<div class="modal-content">
			<form class="form-horizontal recordEditView" id="QuickCreate" name="QuickCreate" method="post" action="index.php">
				...
				<div class="modal-body">
					...
					<input type="hidden" name="action" value="KanbanGridAjax">
					<input type="hidden" name="mode" value="QuickAjax">
					...
					<div class="quickCreateContent">
						<table class="massEditTable table no-border">
							<tr>
								...
								{assign var="referenceListCount" value=ParsVT_Module_Model::php7_count($referenceList)}
								...
								<td class='fieldLabel col-lg-2'>
									{if $isReferenceField neq "reference"}<label class="muted pull-right">{/if}
										...
										{else if $FIELD_MODEL->get('uitype') eq '83'}
											...
											{if $TAXCLASS_DETAILS}
												{assign 'taxCount' ParsVT_Module_Model::php7_count($TAXCLASS_DETAILS)%2}
												{if $taxCount eq 0}
													{if $COUNTER eq 2}
														{assign var=COUNTER value=1}
													{else}
