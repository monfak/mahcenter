<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-top:0px;">
    <h4>{vtranslate('Navatel Settings', $QUALIFIED_MODULE)}</h4>
    <hr>
</div>
...
{assign var=CURRENT_USER_MODEL value=Users_Record_Model::getCurrentUserModel()}
    ...
    {if $PBXMANAGER}
        ...
        <form id="AsteriskCTIForm" class="form-horizontal" action='index.php' method='POST'>
            ...
            <input type="hidden" name="mode" value="NavaTel"/>
            ...
            <div class="blockData">
                <table class="table table-bordered blockContainer showInlineTable equalSplit">
                    <thead>
                        <tr class="active">
                            <th class="" colspan="4"<img src="layouts/{vglobal('default_layout')}/modules/ParsVT/images/helpinfo.png"> &nbsp;{vtranslate('NavaTel Configration', $MODULE)}</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr {if $FIELD_NAME eq 'vtigersecretkey' or $FIELD_NAME eq 'webappurl' or $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'} class="hide" {/if}>
                                ...
                                <td style="word-wrap:break-word;">
                                    {if $FIELD_NAME eq 'vtigersecretkey'}
                                        ...
                                        {/if}
                                    {/if}
                                    {if $FIELD_VALUE[0] eq 'textarea'}
                                        <textarea class="inputElement fieldValue"  rows="8" cols="100"  style=" text-align: left;  max-width: 300px; direction:ltr;font-family:'arial', Times, serif; font-size:9px;letter-spacing: 2px;"  name="{$FIELD_NAME}" data-rule-required="true">{$FIELD_VALUE[1]}</textarea>
                                    {else}
                                        <input class="inputElement fieldValue"  style="direction: ltr; text-align: left; max-width: 300px" type="{$FIELD_VALUE[0]}" name="{$FIELD_NAME}" data-rule-required="true"   value="{if $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}from-internal{else if $FIELD_NAME eq 'vtigersecretkey'}{$VTIGERSECRETKEY}{elseif $FIELD_NAME eq 'webappurl'}{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Navatel/{else}{$FIELD_VALUE[1]}{/if}"/>
                                    {/if}
                                </td>
                            </tr>
                        {/foreach}
                        ...
                        <tr>
                            ...
                            <td class="row-fluid medium">
                                <input class="inputElement fieldValue"
                                       ...
                                       value="{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Navatel/CDR/{$VTIGERSECRETKEY}/" readonly>
                            </td>
                        </tr>
