                        ...
                        <tr {if $FIELD_NAME eq 'vtigersecretkey' or $FIELD_NAME eq 'webappurl' or $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}class="hide" {/if}>
                            ...
                            <td style="word-wrap:break-word;">
                                {if $FIELD_NAME eq 'vtigersecretkey'}
                                    ...
                                    {/if}
                                {/if}
                                <input class="inputElement fieldValue"  style="direction: ltr; text-align: left; max-width: 300px" type="{$FIELD_VALUE[0]}" name="{$FIELD_NAME}" data-rule-required="true"  {if $FIELD_NAME eq 'ring_url'} placeholder="http://YourRingURL/" {/if} value="{if $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}from-internal{elseif $FIELD_NAME eq 'vtigersecretkey'}{$VTIGERSECRETKEY}{elseif $FIELD_NAME eq 'webappurl'}{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Ring/{else}{$FIELD_VALUE[1]}{/if}"/>
                            </td>
                        </tr>
                    {/foreach}
                    <tr>
