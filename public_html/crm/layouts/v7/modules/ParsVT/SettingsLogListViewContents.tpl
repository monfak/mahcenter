...
{strip}
    ...
    <div class="clearfix">
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
            <h3 style="margin-top: 0px;">{vtranslate('Settings Log Tracker', $QUALIFIED_MODULE)}</h3>
        </div>
    </div>
    <hr>
                                ...
                                <tr class="searchRow listViewSearchContainer">
                                    <th class="inline-search-btn">
                                        <div class="table-actions" style="width : auto !important; display: flex">
                                            <button class="searchBtn btn btn-primary"  data-trigger="settingslog_listSearch">
                                                <i class="fa fa-search"></i> {vtranslate('LBL_SEARCH', $MODULE)}
                                            </button>
                                            <button class="searchAndClearButton btn btn-danger {if !$SEARCH_DETAILS ||count($SEARCH_DETAILS) eq 0}hide{/if}" style="width: auto !important;" data-trigger="settingslog_ClearSearch"><i class="fa fa-close"></i></button>
                                        </div>      
                                    </th>
                                    {foreach key=LISTVIEW_HEADER_NAME item=LISTVIEW_FIELD_INFO from=$FIELDS_INFO}
                                        <th>
                                            {if $LISTVIEW_HEADER_NAME eq 'changedon'}
                                                <input type="text" name="changedon" placeholder="{vtranslate('LBL_SELECT_DATE_RANGE',$MODULE)}" class="listSearchContributor inputElement dateField" data-date-format="{$CURRENT_USER_MODEL->get('date_format')}" data-calendar-type="range" value="{$SEARCH_DETAILS[$LISTVIEW_HEADER_NAME]}"  data-field-type="datetime" id="settingslog_{$LISTVIEW_HEADER_NAME}"/>
                                            {else if $LISTVIEW_FIELD_INFO['type'] eq 'picklist'}
                                                {assign var=PICKLIST_VALUES value=ParsVT_SettingsLog_View::getSearchableValues($LISTVIEW_HEADER_NAME)}
                                                <select class="select2 col-lg-11 col-md-11 col-sm-11 listSearchContributor" name="{$LISTVIEW_HEADER_NAME}" id="settingslog_{$LISTVIEW_HEADER_NAME}" >
                                            ...
                                            {else}
                                                <input type="text" name="{$LISTVIEW_HEADER_NAME}" class="listSearchContributor inputElement" value="{$SEARCH_DETAILS[$LISTVIEW_HEADER_NAME]}" id="settingslog_{$LISTVIEW_HEADER_NAME}"/>
                                            {/if}
                                        </th>
                                    {/foreach}
                                </tr>
