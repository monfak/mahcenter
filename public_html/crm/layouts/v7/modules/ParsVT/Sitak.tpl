<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-top:0px;">
    <h4>{vtranslate('Sitak Settings', $QUALIFIED_MODULE)}</h4>
    <hr>
</div>
<div class="col-lg-12" style="margin-top: 10px">
    ...
    {if $PBXMANAGER}
        ...
        <form id="AsteriskCTIForm" class="form-horizontal" action='index.php' method='POST'>
            ...
            <input type="hidden" name="mode" value="Sitak"/>
            <input type="hidden" name="id" value="{$RECORD_ID}">
            <div class="blockData">
                <table class="table table-bordered blockContainer showInlineTable equalSplit">
                    <thead>
                    <tr class="active">
                        <th class="" colspan="4"<img src="layouts/{vglobal('default_layout')}/modules/ParsVT/images/helpinfo.png"> &nbsp;{vtranslate('Sitak Settings', $MODULE)}</th>
                    </tr>
                    </thead>
                    <tbody>
                    {foreach item=FIELD_VALUE key=FIELD_NAME from=$FIELDS}
                        <tr {if $FIELD_NAME eq 'vtigersecretkey' or $FIELD_NAME eq 'webappurl' or $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}class="hide" {/if}>
                            ...
                            <td style="word-wrap:break-word;">
                                <input class="inputElement fieldValue"  style="direction: ltr; text-align: left; max-width: 300px" type="{$FIELD_VALUE[0]}" name="{$FIELD_NAME}" data-rule-required="true"  {if $FIELD_NAME eq 'sitak_url'} placeholder="http://YourSitakURL/connect-CRM/" {/if} value="{if $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}from-internal{elseif $FIELD_NAME eq 'vtigersecretkey'}{if !empty($FIELD_VALUE[1])}{$FIELD_VALUE[1]}{else}{PBXManager_Server_Model::generateVtigerSecretKey()}{/if}{elseif $FIELD_NAME eq 'webappurl'}{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Sitak/{else}{$FIELD_VALUE[1]}{/if}"/>
                            </td>
                        </tr>
                    {/foreach}
                    <tr>
                    ...
                    <tr>
                        <td class="fieldLabel control-label"
                            style="width:25%">{vtranslate('Sitak API URL',$QUALIFIED_MODULE)}</td>
                        <td class="row-fluid medium">
                            <input class="inputElement fieldValue"
                                   style="direction: ltr; text-align: left;" type="text"
                                   value="{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Sitak/CDR/" readonly>
                        </td>
                    </tr>
                    <tr>
                        <td class="fieldLabel control-label"
                            style="width:25%">{vtranslate('Sitak API Username',$QUALIFIED_MODULE)}</td>
                        <td class="row-fluid medium">
                            <input class="inputElement fieldValue"
                                   style="direction: ltr; text-align: left;" type="text"
                    ...
                    <tr>
                        <td class="fieldLabel control-label"
                            style="width:25%">{vtranslate('Sitak API password',$QUALIFIED_MODULE)}</td>
                        <td class="row-fluid medium">
                            <input class="inputElement fieldValue"
                                   style="direction: ltr; text-align: left;" type="text"
