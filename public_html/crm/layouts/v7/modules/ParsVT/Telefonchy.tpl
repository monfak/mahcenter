<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="margin-top:0px;">
    <h4>{vtranslate('Telefonchy Settings', $QUALIFIED_MODULE)}</h4>
    <hr>
</div>
    ...
    {if $PBXMANAGER}
            ...
            {if $SUCCESS}
                <table class="table table-bordered blockContainer showInlineTable equalSplit">
                    ...
                    <tr>
                        <td class="fieldLabel medium">
                            <label class="muted pull-right marginRight10px">{vtranslate('Account Information', $MODULE)}</label>
                        </td>
                        <td  class="alert alert-success text-center" colspan="3">

                                {foreach item=LINE from=$INFO['trunks']}
                                    <div>{vtranslate('Line number:', $MODULE)} <span style="border-radius: 3px; background: #fff; color: #ff6a00;">{$LINE['number']}</span> </div>
                                {/foreach}
                            {foreach item=LINE from=$INFO['trunks']}
                                    {if $LINE['credit']['value'] gt 0}
                                    <div>{vtranslate('Line credit:', $MODULE)} <span style="border-radius: 3px; background: #fff; color: {if $LINE['credit']['value'] eq 0} red {else} blue {/if};"> {CurrencyField::convertToUserFormat($LINE['credit']['value'], null, true)}</span></div>
                                    {/if}
                                {/foreach}
                            </div>
                        </td>
        ...
        <form id="AsteriskCTIForm" class="form-horizontal" action='index.php' method='POST'>
            ...
            <input type="hidden" name="mode" value="Telefonchy"/>
            <input type="hidden" name="id" value="{$RECORD_ID}">
            <div class="blockData">
                <table class="table table-bordered blockContainer showInlineTable equalSplit">
                    <thead>
                    <tr class="active">
                        <th class="" colspan="4"<img src="layouts/{vglobal('default_layout')}/modules/ParsVT/images/helpinfo.png"> &nbsp;{vtranslate('Telefonchy Settings', $MODULE)}</th>
                    </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr {if $FIELD_NAME eq 'vtigersecretkey' or $FIELD_NAME eq 'webappurl' or $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}class="hide" {/if}>
                            ...
                            <td style="word-wrap:break-word;">
                                ...
                                <input class="inputElement fieldValue"  style="direction: ltr; text-align: left; max-width: 300px" type="{$FIELD_VALUE[0]}"
                                       name="{$FIELD_NAME}" data-rule-required="true"   value="{if $FIELD_NAME eq 'outboundcontext' or $FIELD_NAME eq 'outboundtrunk'}from-internal{else if $FIELD_NAME eq 'vtigersecretkey'}{$VTIGERSECRETKEY}{elseif $FIELD_NAME eq 'webappurl'}{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Telefonchy/{else}{$FIELD_VALUE[1]}{/if}"/>
                            </td>
                        </tr>
                    {/foreach}
                    <tr>
               							...
               							<select class="inputElement fieldValue select2 {$ASSIGNED_USER_ID}" data-validation-engine="validate required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-name="{$ASSIGNED_USER_ID}" name="analogpopupusers[]" data-fieldinfo='{$FIELD_INFO}'  multiple  style="max-width: 300px">
               								<optgroup label="{vtranslate('LBL_USERS')}">
               									{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
               										<option value="{$OWNER_ID}" data-picklistvalue= '{$OWNER_NAME}'{foreach item=USER from=$POPUP_USERS}{if $USER eq $OWNER_ID } selected {/if}{/foreach}
                    ...
                    <tr>
                        ...
                        <td class="row-fluid medium">
                            <input class="inputElement fieldValue"
                                   ...
                                   value="{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Telefonchy/CDR/" readonly>
                        </td>
                    </tr>
                    <tr>
                        <td class="fieldLabel control-label"
                            style="width:25%">{vtranslate('NewState Event API URL',$QUALIFIED_MODULE)}</td>
                        <td class="row-fluid medium">
                            <input class="inputElement fieldValue"
                                   style="direction: ltr; text-align: left;" type="text"
                                   value="{vglobal('site_URL')}modules/ParsVT/ws/API/V2/PBX/Telefonchy/NewState/" readonly>
                        </td>
                    </tr>
                    <tr>
