...
kbd {
    ...
    <div class="installationContents" style="padding-left: 3%;padding-right: 3%">
        <div class="padding1per" style="border:1px solid #ccc;padding: 1%;">
            ...
            <div class="row {if $WHITELABEL}hide{/if}" style="margin-bottom:10px;">
                <span class="col-sm-2" style="text-align: right"><strong>{vtranslate('ChangeLogs',$QUALIFIED_MODULE)}:</strong></span>
                <span class="col-sm-6"><a href="http://parsvt.com/changelogs/" target="_blank"><strong>{vtranslate('LBL_VIEW_DETAILS',$QUALIFIED_MODULE)}</strong></a></span>
            </div>
            <div class="row" style="margin-bottom:10px;">
            ...
            <div class="row" style="margin-bottom:10px;">
                <span class="col-sm-2" style="text-align: right"><strong>{vtranslate('Current ionCube Loader Version:',$QUALIFIED_MODULE)}</strong></span>
                <span class="col-sm-6">{$LOADERVERSION}</span>
            </div>
            <div class="row" style="margin-bottom:10px;">
            ...
            {if $LOADER}
                ...
                {/if}
            {else}
                <div class="control-group alert alert-danger" style="text-align: center;">
                    <div>
                        <span>
                            {vtranslate('The installed version of the Loader cannot run files produced by the most recent ionCube Encoder. <br />You will need a version 12 ionCube Loader to use ParsVT module.',$QUALIFIED_MODULE)}
                            <br />
                            <button class="btn btn-danger" type="button" onclick="window.open('modules/ParsVT/phpinfo.php')"><strong>{vtranslate('LBL_UPGRADE', $QUALIFIED_MODULE)}</strong></button>
                        </span>
                    </div>
                </div>
            {/if}
            {if $UpdateStatus && !$WHITELABEL}
