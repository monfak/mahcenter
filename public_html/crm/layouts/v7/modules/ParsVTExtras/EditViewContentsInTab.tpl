...
{strip}
    ...
    {assign var=COMBINE_TAB_LIST value=ParsVTExtras_Module_Model::getCombineTabs($MODULE)}
    ...
    {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
        {assign var=TAB_INFO value=ParsVTExtras_Module_Model::checkIsTab($BLOCK_LIST[$BLOCK_LABEL]->get('id'))}
        ...
        {if $BLOCK_FIELDS|@count gt 0}
            {include file="ChildEditViewContentsInTab.tpl"|vtemplate_path:"ParsVTExtras" BLOCK_FIELDS = $BLOCK_FIELDS RECORD_STRUCTURE =$RECORD_STRUCTURE BLOCK_LABEL = $BLOCK_LABEL  NONE_TAB = true}
        {/if}
    {/foreach}
    {*End un_tab block*}
    {*Start tab block*}
    {if $TAB_ARRAYS_BLOCK_FIELDS|@count gt 0}
        <div id="vd_tabs" class="fieldBlockContainer tabs">
            <div class="related-tabs">
                <ul class="tab-links nav nav-tabs">
                    ...
                    {if $COMBINE_TAB_LIST|@count gt 0}
                        ...
                        {/foreach}
                    {/if}
                    {assign var=TAB_LIST_WITH_ORDER value=ParsVTExtras_Module_Model::getTabsLabelWithOrder($MODULE)}
                ...
                {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$TAB_ARRAYS_BLOCK_FIELDS}
                    ...
                    {if $BLOCK_FIELDS|@count gt 0 && $TAB_INFO['parent_tab'] eq "" && $TAB_INFO['parent_tab'] eq null}
                        ...
                        <div class='fieldBlockContainer tab' id="block-{$BLOCK_LIST[$BLOCK_LABEL]->get('id')}">
                            {include file="ChildEditViewContentsInTab.tpl"|vtemplate_path:"ParsVTExtras" BLOCK_FIELDS = $BLOCK_FIELDS RECORD_STRUCTURE =$RECORD_STRUCTURE BLOCK_LABEL = $BLOCK_LABEL}
                        </div>
                    {/if}
                    {/if}
                    ...
                    <div class='fieldBlockContainer tab' id="block-{$TAB_KEY}">
                        {foreach key=BLOCK_LABEL2 item=BLOCK_FIELDS2 from=$TAB_CHILD_ARRAYS_BLOCK_FIELDS name=blockIterator}
                            ...
                            {if $BLOCK_FIELDS2|@count gt 0 &&  $CHILD_TAB_INFO['parent_tab'] eq {$TAB_NAME}}
                                {include file="ChildEditViewContentsInTab.tpl"|vtemplate_path:"ParsVTExtras" BLOCK_FIELDS = $BLOCK_FIELDS2 RECORD_STRUCTURE =$RECORD_STRUCTURE BLOCK_LABEL = $BLOCK_LABEL2}
                            {/if}
                        {/foreach}
                    </div>
                {/foreach}
