...
{strip}
    ...
    {if $FIELD_NAME|strstr:'rf_pcf_mrf' && $FIELD_NAME|strpos:'rf_pcf_mrf' == '0'}
        ...
        {/if}
        {assign var="RELATED_LIST_INFO" value=ParsVT_MultiReference_Helper::getRelatedInfo($RECORDID, $FIELD_MODEL->get('id'))}
        <div class="referencefield-wrapper">
            ...
            {if {$REFERENCE_LIST_COUNT} gt 1}
                ...
                {/if}
            {/if}
            <input name="{$FIELD_NAME}" type="hidden" value="{$FIELD_MODEL->get('fieldvalue')}" class="sourceField" data-displayvalue='{$FIELD_MODEL->getEditViewDisplayValue($FIELD_MODEL->get('fieldvalue'))}' data-fieldinfo='{$FIELD_INFO}' data-field-id= '{$FIELD_MODEL->get('id')}' data-multiple='true'/>
            <div class="input-group">
                ...
                />
                <span class="input-group-addon relatedMultiPopup cursorPointer" title="{vtranslate('LBL_SELECT', $MODULE)}" style="height:auto;width: 30px;">
   				<i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_select" class="fa fa-search"></i>
   			</span>
                {if $smarty.request.view eq 'Edit'}
                    <span class="input-group-addon createMultiReferenceRecord cursorPointer clearfix" title="{vtranslate('LBL_CREATE', $MODULE)}">
   				<i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_create" class="fa fa-plus"></i>
   			</span>
                {/if}
                <input type="hidden" name="related{$REFERENCED_MODULE_NAME}Info" data-value='{json_encode($RELATED_LIST_INFO, $smarty.const.JSON_HEX_APOS)}' />
            </div>
