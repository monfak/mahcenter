...
{if $LISTVIEW_ENTRIES_COUNT eq '0'}
	<table class="emptyRecordsDiv">
		<tbody>
			<tr>
				<td>
		 {if $smarty.cookies.ParsvtLang == 'fa_ir' || $smarty.cookies.ParsvtLang == 'fa_af'}
					{assign var=SINGLE_MODULE value="SINGLE_$MODULE"}
					{vtranslate('PVT_NO')} {vtranslate($SINGLE_MODULE, $MODULE)} {vtranslate('PVT_YE2')} {vtranslate('PVT_FOUND')}.{if $IS_MODULE_EDITABLE} {vtranslate('PVT_YOU_CAN')}  {vtranslate($SINGLE_MODULE, $MODULE)} <a href="{$MODULE_MODEL->getCreateRecordUrl()}">{vtranslate('LBL_CREATE')}</a> {vtranslate('PVT_DO')} <!--{vtranslate($MODULE, $MODULE)}-->{else}<!--{vtranslate($SINGLE_MODULE, $MODULE)}-->{/if}
                {else}
					{assign var=SINGLE_MODULE value="SINGLE_$MODULE"}
					{vtranslate('LBL_NO')} {vtranslate($MODULE, $MODULE)} {vtranslate('LBL_FOUND')}.{if $IS_MODULE_EDITABLE} {vtranslate('LBL_CREATE')} <a href="{$MODULE_MODEL->getCreateRecordUrl()}">{vtranslate($SINGLE_MODULE, $MODULE)}</a> {vtranslate($MODULE, $MODULE)}{else}{vtranslate($SINGLE_MODULE, $MODULE)}{/if}
                {/if}
