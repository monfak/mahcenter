...
class EnhancedQueryGenerator extends QueryGenerator {
	...
	public function getFromClause() {
			...
			if (empty($referenceParentFieldName)) {
				...
				if ($fieldType == 'reference') {
					...
					if ($fieldName == 'roleid' && $baseModule == 'Users') {
						$tableJoinMapping['vtiger_role'] = 'INNER JOIN';
						$tableList['vtiger_role'] = 'vtiger_role';
					} else if (strpos(strtolower($fieldName), 'rf_pcf_mrf') !== false) {
					$multiRefFields[$fieldName] = $field;
					}
				} else if ($fieldType == 'owner') {
					$tableList['vtiger_users'] = 'vtiger_users';
					$tableList['vtiger_groups'] = 'vtiger_groups';
        ...
        } else if (self::php7_count($ownerFields)>0){
		...
		if (!empty($multiRefFields)) {
                    foreach ($multiRefFields as $multiRefField) {
                        $field_related_modules = $multiRefField->getReferenceList();
                        foreach ($field_related_modules as $field_related_module) {
                            $meta       = $this->getMeta($field_related_module);
                            $related_table = $meta->getEntityBaseTable();
                            $related_table_index = $meta->getObectIndexColumn();
                            $sql        .= "inner JOIN vtiger_multireference_rel AS Multireference_$field_related_module on Multireference_$field_related_module.targetid=" . $baseTable . ".$baseTableIndex inner JOIN $related_table  ON $related_table.$related_table_index = Multireference_$field_related_module.crmid ";
                        }
                    }
                }
		$this->fromClause = $sql;
		...
		foreach ($this->conditionals as $index => $conditionInfo) {
			...
			if ($operator == 'between' && $this->isDateType($field->getFieldDataType())) {
				...
				if (self::php7_count($end) == 2) {
                    ...
                    if ($date_calendar == 'jalali') {
                        $conditionInfo['value'][1] = ParsVT_Adaptive_Helper::jdate('Y-m-d H:i:s', strtotime($conditionInfo['value'][1]));
                    }
				}
			}
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($baseFieldName, $this->referenceFieldList)) {
						...
						if($fieldName == 'currency_id' && is_numeric($conditionInfo['value'])){
							$columnSql = "vtiger_currency_info$parentReferenceField$fieldName.id";
		                            ...
		                            foreach ($module_ref_fields as $module_ref_field) {
		                                $ref_meta            = $this->getMeta($module_ref_field);
		                                $ref_fields = $ref_meta->getModuleFields();
		                                $nameFields = explode(",",$ref_meta->getNameFields());
		                                if (count($nameFields) > 1) {
		                                    foreach ($nameFields as &$nameField) {
		                                        $fieldInstance = $ref_fields[$nameField];
		                                        $nameField = $fieldInstance->getTableName() . '.' . $nameField;
		                                    }
		                                    $columnSql = 'CONCAT('.implode(",", $nameFields).")";
		                                } else {
		                                    $fieldInstance = $ref_fields[$ref_meta->getNameFields()];
		                                     $columnSql = $fieldInstance->getTableName().".". $ref_meta->getNameFields();
		                                }
		                            }
					}else {
                        			$trim = '';
					...
					if (($fieldName == 'birthday'  || substr( $fieldName, 0, 10 ) === 'cf_pcf_adf') && !$this->isRelativeSearchOperators($conditionInfo['operator'])) {
