...
class QueryGenerator {
	...
	public function __construct($module, $user) {
		...
		$this->ownerFields = $this->getOwnerFields();//add by Mahdi Bagheri
		$this->columns = null;
    ...
    public static function php7_sizeof($value) {
    	// PHP 8.x does not allow sizeof(null)
    	return self::php7_count($value);
    }
	
	//add by Mahdi Bagheri
	public function getOwnerFields() {
		...
		if($module == 'Calendar') {
			...
			$eventModuleOwnerFieldList = $eventmoduleMeta->getOwnerFields();
			$ownerFields = array_unique(array_merge($ownerFields, $eventModuleOwnerFieldList));
		}
		return $ownerFields;
	}
	...
	public function initForCustomViewById($viewId) {
		...
		if($this->conditionInstanceCount <= 0 && is_array($this->advFilterList) && self::php7_count($this->advFilterList) > 0) {
			$this->startGroup('');
		} elseif($this->conditionInstanceCount > 0 && is_array($this->advFilterList) && self::php7_count($this->advFilterList) > 0) {
			$this->addConditionGlue(self::$AND);
		}
		if(is_array($this->advFilterList) && self::php7_count($this->advFilterList) > 0) {
	...
	public function parseAdvFilterList($advFilterList, $glue=''){
		...
		foreach ($advFilterList as $groupindex=>$groupcolumns) {
			...
			if(self::php7_count($filtercolumns) > 0) {
				$this->startGroup('');
				foreach ($filtercolumns as $index=>$filter) {
					...
					if(($nameComponents[4] == 'D' || $nameComponents[4] == 'DT') && in_array($filter['comparator'], $dateSpecificConditions)) {
						...
						for($i=0;$i<self::php7_count($value);$i++){
...
/**PVTPATCHER-6996558312B57033EFFE51CD22A1A428-START-utils**/
/** Don't remove the Start and Finish Markup! Modified: 2024-02-27 21:58:18 **/
foreach ($this->fields as $field) {
            if ($field == 'campaignrelstatus') continue;
/** REPLACED-6996558312B57033EFFE51CD22A1A428// foreach ($this->fields as $field) { **/
/**PVTPATCHER-6996558312B57033EFFE51CD22A1A428-FINISH**/
...
/**PVTPATCHER-B2975BCF3FCF96069A4A0B161FD93DA0-START-utils**/
/** Don't remove the Start and Finish Markup! Modified: 2024-02-27 21:58:18 **/
if ($fieldName == 'id' || $fieldName == 'campaignrelstatus') {
/** REPLACED-B2975BCF3FCF96069A4A0B161FD93DA0// if ($fieldName == 'id') {**/
/**PVTPATCHER-B2975BCF3FCF96069A4A0B161FD93DA0-FINISH**/
				continue;
			}
		...
		$ownerFields = $this->meta->getOwnerFields();
		if (self::php7_count($ownerFields) > 0) {
	...
	public function getWhereClause() {
        ...
        if (class_exists('ParsVT_Adaptive_Helper') && (isset($current_user->parsvt_calendar) ||  $default_system_date)) {
           if ($current_user->parsvt_calendar == 'jalali') {
               $date_format = 'pdate_format';
               $date_calendar = 'jalali';
           } elseif ($default_system_date == 'jalali') {
               $date_format = 'pdate_format';
               $date_calendar = 'jalali';
           }
        }
		...
		foreach ($this->conditionals as $index=>$conditionInfo) {
			...
			//Added to skip if module not available for reference fields
			$skipReferenceField = array('roleid');
			// added to skip if module is disabled for reference fields
			if (in_array($baseFieldName, $this->referenceFieldList) && !in_array($baseFieldName, $skipReferenceField)) {
				$moduleList = $this->referenceFieldInfoList[$fieldName];
				if(empty($moduleList)){
					continue;
				}
			...
			foreach ($valueSqlList as $valueSql) {
				if (in_array($fieldName, $this->referenceFieldList)) {
				...
				}  else if($field->getFieldDataType() == 'coowner') {
					$value = $conditionInfo['value'];
					$operator = $conditionInfo['operator'];
					if($operator == 'ny' || ($operator == 'n' && !$value)) {
						// is not empty (or) not equal to ''
						$fieldSql .= "$fieldGlue $tableName.coowners IS NOT NULL AND $tableName.coowners <> '' ";
					} else if($operator == 'y' || ($operator == 'e' && !$value)) {
						// is empty (or) equal to ''
						$fieldSql .= "$fieldGlue trim($tableName.coowners) IS NULL OR trim($tableName.coowners) = '' ";
					} else if($operator == 'n' && $value) {
					$coowners = explode(',', $value);
					$condSql = array();
					foreach($coowners as $coowner) {
						$condSql[] = "!FIND_IN_SET($coowner, $tableName.coowners)";
					}
					$fieldSql .= "$fieldGlue (".implode(' AND ', $condSql).")";
				} else if($value) {
					$coowners = explode(',', $value);
					$condSql = array();
					foreach($coowners as $coowner) {
					...
					$fieldSql .= "$fieldGlue (".implode(' OR ', $condSql).")";
				}
					...
					if($operator == 'bw') {
						...
						if(self::php7_count($startDateValue) == 2 && self::php7_count($endDateValue) == 2) {
					...
					if($operator == 'bw') {
						...
						if($startDateValue[1] == '00:00:00' && ($endDateValue[1] == '00:00:00' || $endDateValue[1] == '23:59:59')) {
							...
							* 1) createdtime, modifiedtime -> uitype 70
							* 2) work orders(scheduled_start_datetime) & Timelogs(startedon, endedon) -> uitype 118
							* we have to cast as DATETIME else comparison will fail
							*Edit by Mahdi Bagheri
							*$fieldSql .= "$fieldGlue CAST(".$field->getTableName().'.'.$field->getColumnName()." AS DATE) $valueSql";
							*/
							$dateTimeTypes = array(70, 118);
							$castType = (in_array($field->getUIType(), $dateTimeTypes)) ? 'DATETIME' : 'DATE';
							$fieldSql .= "$fieldGlue CAST(".$field->getTableName().'.'.$field->getColumnName()." AS $castType) $valueSql";
                        } else {
				    ...
				    //by hamid on 13980612
					if(($fieldName == 'birthday'  || substr( $fieldName, 0, 10 ) === 'cf_pcf_adf') && !$this->isRelativeSearchOperators(
	...
	protected function getConditionValue($value, $operator, $field) {
		...
		if(is_string($value) && $this->ignoreComma == false && substr($field->getFieldName(), 0, 10) !== 'cf_pcf_icf') {
			//add by Mahdi Bagheri
			$commaSeparatedFieldTypes = array('picklist', 'multipicklist', 'metricpicklist', 'owner', 'ownergroup', 'date', 'datetime', 'time');
			...
			if(in_array($operator, array('s', 'ew', 'c', 'k')) || in_array($field->getFieldDataType(), $commaSeparatedFieldTypes) || ($field->getFieldName() == 'salutationtype') || ($field->getFieldDataType()=='reference' && in_array('Users', $field->getReferenceList()))) {
                ...
                $valueArray = str_getcsv($value);//add by Mahdi Bagheri
		...
		if($operator == 'between' || $operator == 'bw' || $operator == 'notequal') {
			if($field->getFieldName() == 'birthday' || substr( $field->getFieldName(), 0, 10 ) === 'cf_pcf_adf') {
				$valueArray[0] = getValidDBInsertDateTimeValue($valueArray[0]);
				$valueArray[1] = getValidDBInsertDateTimeValue($valueArray[1]);
                if ($date_calendar == 'jalali') {
                    $jalali_format = 'Y-m-d H:i:s';
                    if (count(explode(" ",$valueArray[0])) == 1) {
                        $jalali_format = "Y-m-d";
                    }
                    $valueArray[0] = ParsVT_Adaptive_Helper::jdate( $jalali_format, strtotime($valueArray[0]));
                    $valueArray[1] = ParsVT_Adaptive_Helper::jdate( $jalali_format,strtotime($valueArray[1]));
				...
				if($this->isDateType($field->getFieldDataType())) {
					...
					if($operator == 'between' && self::php7_count($start) == 2 && self::php7_count($end) == 2){
...
/**PVTPATCHER-F708494B3CA7DF0A8E0A9739DEACF91C-START-advcf**/
/** Don't remove the Start and Finish Markup! Modified: 2024-02-27 21:58:08 **/
        else if (in_array($field->getUIType(), array(7,9,71,72))) {
            $valueArray = explode(',' , $value);
        }
/**PVTPATCHER-F708494B3CA7DF0A8E0A9739DEACF91C-FINISH**/
        ...
        foreach ($valueArray as $value) {
            ...
            if ($operator == 'empty' || $operator == 'y') {
                ...
                } else if ($sqlFieldDataType == 'datetime' || $sqlFieldDataType == 'customdatetime') {
			...
			$trimmedValue = is_array($value) ? NULL : trim($value);
				...
				($operator == 'e' || $operator == 'n') && ($field->getFieldDataType() != 'owner' && $field->getFieldDataType() != 'ownergroup')) {//edit by Mahdi Bagheri
				if($operator == 'e'){
					$sql[] = "IS NULL";
	                                $sqlFieldDataType = $field->getFieldDataType();
	                                if($sqlFieldDataType == 'date' || $sqlFieldDataType == 'birthday'){
				...
				$values = explode(' ',$value);
				if(($operator == 'a' || $operator == 'b') && self::php7_count($values) == 2){
					if($operator == 'a'){
			...
			} else if ($field->getFieldDataType() === 'multicurrency') {//add by Mahdi Bagheri
			...
			if(($field->getFieldName() == 'birthday'  || substr( $field->getFieldName(), 0, 10 ) === 'cf_pcf_adf') && !$this->isRelativeSearchOperators(
					$operator)) {
                ...
                $value = ParsVT_Adaptive_Helper::jdate($jalali_format, strtotime($value));
            ...
            switch ($operator) {
                ...
                case 'ew':
                    ...
                    break;
                case 'c':
                    if ($field->getFieldName() == 'currency_id' && is_numeric($value)) {//add by Mahdi Bagheri
                        $sqlOperator = "=";
                        break;
                    }
                    $sqlOperator = "LIKE";
                ...
                case 'k':
                    ...
                    //$value = "%$value%";
                    $value = "%" . trim($value) . "%";//add by Mahdi Bagheri
                    break;
                case 'l':
                    $sqlOperator = "<";
                    break;
			...
			if( ($field->getFieldName() != 'birthday' || (($field->getFieldName() == 'birthday' || substr( $field->getFieldName(), 0, 10 ) === 'cf_pcf_adf')
							&& $this->isRelativeSearchOperators($operator)))){
				//add by (Mahdi Bagheri)
				if($field->getFieldDataType() !== 'integer' && $field->getFieldName() != 'birthday' && substr( $field->getFieldName(), 0, 10 ) != 'cf_pcf_adf'){
					$value = "'$value'";
				}
			}
            ...
            if (substr($field->getFieldName(), 0, 10) === 'cf_pcf_icf') {
                $value = $valueArray[0];
                $values = explode(',', $value);
                if ($operator == 'e' || $operator == 'c') {
                    ...
                    $sql[] = 'IN (SELECT vtiger_parsvt_cities.LocationId FROM vtiger_parsvt_cities WHERE vtiger_parsvt_cities.Parent IN (' . $valueArray[0] . '))';
                } else if ($operator == 'ne') {
                    if (count($values) > 1) {
                        foreach ($values as $val) {
                            $sql[] = '<> ' . trim($val);
                    ...
                    $sql[] = 'NOT IN (SELECT vtiger_parsvt_cities.LocationId FROM vtiger_parsvt_cities WHERE vtiger_parsvt_cities.Parent IN (' . $valueArray[0] . '))';
	...
	*new add function (Mahdi bagheri)
	*/
	...
	public function hasConditionals() {
		if(self::php7_count($this->conditionals) > 0) {
			return true;
		}
		return false;
	}
	...
	public function addUserSearchConditions($input) {
		...
		if(isset($input['searchtype']) && $input['searchtype']=='advance') {
			...
			if(empty($advft_criteria) || self::php7_count($advft_criteria) <= 0) {
				return ;
			}

			$advfilterlist = getAdvancedSearchCriteriaList($advft_criteria, $advft_criteria_groups, $this->getModule());

			if(empty($advfilterlist) || self::php7_count($advfilterlist) <= 0) {
				return ;
			}
			...
			$noOfConditions = self::php7_count($conditionList);
			$noOfRelatedConditions = self::php7_count($relatedConditionList);
			...
			if(isset($input['search_text']) && $input['search_text']!="") {
				...
				if($type == 'picklist') {
					...
					if(self::php7_sizeof($mod_keys) >= 1) {
