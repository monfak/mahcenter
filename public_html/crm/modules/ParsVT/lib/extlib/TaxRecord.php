...
class Inventory_TaxRecord_Model extends Vtiger_Base_Model {
	...
	*/
	...
	/**
	 * Function to get selected default tax mode
	 * @return <String> $taxmode
	 */
	...
	/**
	 * Function to update default tax mode
	 * @param <String> $defaultTaxMode
	 * @return <Boolean>
	 */
	public static function updateDefaultTaxMode($defaultTaxMode) {
		$db = PearDatabase::getInstance();
		$inventoryModules = getInventoryModules();
        ParsVT_Tracker_Model::triggerEventHandler('Tax Management', 'vtiger.settings.beforesave.tax_defaultmode', array('id'=>'defaultmode','defaultmode'=>$defaultTaxMode));
		foreach ($inventoryModules as $moduleName) {
			$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
			$fieldModel = Vtiger_Field_Model::getInstance('hdnTaxType', $moduleModel);
			$fieldModel->set('defaultvalue', $defaultTaxMode);
			$fieldModel->save();
		}
        ParsVT_Tracker_Model::triggerEventHandler ('Tax Management', 'vtiger.settings.aftersave.tax_defaultmode', array('id'=>'defaultmode','defaultmode'=>$defaultTaxMode));
		return true;
	}
