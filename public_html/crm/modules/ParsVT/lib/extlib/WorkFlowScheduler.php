    ...
    public function queueScheduledWorkflowTasks()
    {
        ...
        for ($i = 0; $i < $noOfScheduledWorkflows; ++$i) {
            ...
            if ($tasks) {
                ...
                $page = 0;
                do {
                    $records = $this->getEligibleWorkflowRecords($workflow, $page++, 200);
                    if (function_exists('php7_count')) {
    ...
    function addWorkflowConditionsToQueryGenerator($queryGenerator, $conditions)
    {
        ...
        if ($noOfConditions > 0) {
            ...
            foreach ($conditions as $index => $condition) {
                ...
                if ((strpos('birthday', $fieldname) !== false || substr($fieldname, 0, 10) === 'cf_pcf_adf') && in_array($operation, $specialDateComparator)) {
                    $operator = 'e';
                }
