...
class Google_Contacts_Model extends WSAPP_SyncRecordModel {
    ...
    function  __construct($values) {
        $this->data = $values;
        $this->data2 = json_decode(json_encode($this->data) , true);
    }
    /**
    ...
    public function getModifiedTime() {
        return $this->vtigerFormat($this->data2['entity']['updated']['$t']);
    }
    
    function getNamePrefix() {
        $namePrefix = $this->data2['entity']['gd$name']['gd$namePrefix']['$t'];
        return $namePrefix;
    }
    ...
    function getFirstName() {
        $fname = $this->data2['entity']['gd$name']['gd$givenName']['$t'];
        return $fname;
    }
    ...
    function getLastName() {
        $lname = $this->data2['entity']['gd$name']['gd$familyName']['$t'];
        return $lname;
    }
     ...
     */
    function getEmails() {
        $arr = $this->data2['entity']['gd$email'];
        $emails = array();
        if (is_array($arr)) {
     ...
     */
    function getPhones() {
        $arr = $this->data2['entity']['gd$phoneNumber'];
        $phones = array();
        if(is_array($arr)) {
    ...
    function getAddresses() {
        $arr = $this->data2['entity']['gd$structuredPostalAddress'];
        $addresses = array();
        if(is_array($arr)) {
    ...
    function getUserDefineFieldsValues() {
        $fieldValues = array();
        $userDefinedFields = $this->data2['entity']['gContact$userDefinedField'];
        if(is_array($userDefinedFields) && ParsVT_Module_Model::php7_count($userDefinedFields)) {
    ...
    function getUrlFields() {
        $websiteFields = $this->data2['entity']['gContact$website'];
        $urls = array();
    ...
    function getBirthday() {
        return $this->data2['entity']['gContact$birthday']['when'];
    }
    
    function getTitle() {
        return $this->data2['entity']['gd$organization'][0]['gd$orgTitle']['$t'];
    }
    
    function getAccountName($userId) {
        $description = false;
        $orgName = $this->data2['entity']['gd$organization'][0]['gd$orgName']['$t'];
        if(empty($orgName)) {
    ...
    function getDescription() {
        return $this->data2['entity']['content']['$t'];
    }
