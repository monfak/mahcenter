...
function addSignature($contents, $fromname, $fromEmail = '') {
	...
	if ($sign == null) {
		...
		if (version_compare($vtiger_current_version, '7.4.0', '>=')) {
...
function setMailerProperties($mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')
{
	...
	if($to_email != '')
	{
		if(is_array($to_email)) {
			for($j=0,$num=phpmailer7_count($to_email);$j<$num;$j++) {
				$mail->addAddress($to_email[$j]);
			}
		} else {
			$_tmp = explode(",",$to_email);
			for($j=0,$num=phpmailer7_count($_tmp);$j<$num;$j++) {
...
function setMailServerProperties($mail)
{
	...
	if (method_exists('Vtiger_Functions','fromProtectedText')) {
		$mail->Password = Vtiger_Functions::isProtectedText($password) ? Vtiger_Functions::fromProtectedText($password) : $password; // SMTP password
	} else {
...
function setCCAddress($mail,$cc_mod,$cc_val)
{
	...
	if($cc_val != '')
	{
		...
		for($i=0;$i<phpmailer7_count($ccmail);$i++)
		{
			$addr = $ccmail[$i];
...
function getDefaultAssigneeEmailIds($groupId) {
	...
	if($groupId != '') {
		...
		if(phpmailer7_count($userGroups->group_users) == 0) return array();
...
function phpmailer7_count($value) {
