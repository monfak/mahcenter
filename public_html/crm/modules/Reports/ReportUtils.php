...
function getReportFieldValue ($report, $picklistArray, $dbField, $valueArray, $fieldName, $operation = false) {
	...
	if(is_object($field) &&	$field->getUIType() == 401){
	...
	} elseif ( $fieldType == "multicurrency" && ! empty( $value ) ) {
        $fieldvalue = Vtiger_Multicurrency_UIType::getListViewDisplayValue($field->getFieldId(), $value, false, '');
	} elseif ( ($fieldType == "datetime" || $fieldType == "customdatetime") && ! empty( $value ) ) {//by mahdi
    ...
    } elseif ( $fieldInfo['uitype'] == 53 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_atf' ) {
           if (is_numeric($fieldvalue)) {
               if (class_exists('ParsVT_Constant_Model')) {
                   $owner_id = $fieldvalue;
                   $fieldvalue = (is_numeric($owner_id) ? (ParsVT_Constant_Model::getUserFullName($owner_id) != '' ? ParsVT_Constant_Model::getUserFullName($owner_id) : (ParsVT_Constant_Model::getGroupFullName($owner_id) != '' ? ParsVT_Constant_Model::getGroupFullName($owner_id) : $owner_id)) : $owner_id);
               } else {
                   $fieldvalue = getUserFullName(intval($fieldvalue));
               }
           }
   	} elseif ( $fieldInfo['uitype'] == 52 ) {
           if (is_numeric($fieldvalue)) {
               if (class_exists('ParsVT_Constant_Model')) {
                   $owner_id = $fieldvalue;
                   $fieldvalue = (is_numeric($owner_id) ? (ParsVT_Constant_Model::getUserFullName($owner_id) != '' ? ParsVT_Constant_Model::getUserFullName($owner_id) : (ParsVT_Constant_Model::getGroupFullName($owner_id) != '' ? ParsVT_Constant_Model::getGroupFullName($owner_id) : $owner_id)) : $owner_id);
    	...
    	} else if (($fieldType == 'reference' || $fieldType == 'multireference') && substr($fieldInfo['fieldname'], 0, 10) === 'rf_pcf_mrf' && method_exists('ParsVT_MultiReference_Helper', 'getDisplayValue')) {
	        $owner_id = end($valueArray);
	        if (!empty($owner_id) && is_numeric($owner_id)) {
	            $breaks = array('<br />', '<br>', '<br/>');
	            $fieldvalue = ParsVT_MultiReference_Helper::getDisplayValue($owner_id, '', $fieldInfo['fieldname']);
	            $fieldvalue = str_ireplace($breaks, ' - ', $fieldvalue);
	            $fieldvalue = strip_tags($fieldvalue);
	        }
	} else if ( $fieldType=='reference' && $fieldInfo['fieldname'] == 'user'  ) {
        	$owner_id = intval( $fieldvalue );
            $fieldvalue    = $owner_id ? getOwnerName( intval( $owner_id ) ) : '-';
	} elseif ( $fieldInfo['uitype'] == 21 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_arf' ) {
        if (class_exists('ParsVT_Constant_Model')) {
            $fieldvalue = ParsVT_Constant_Model::repeaterView($fieldvalue);
        }
    } elseif ( $fieldInfo['uitype'] == 777 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_icf' ) {
		$fieldvalue = Vtiger_Citiesfield_UIType::getDisplayValue( $fieldvalue );
    } elseif ( $fieldInfo['uitype'] == 777 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_csf' ) {
		$fieldvalue = Vtiger_Countries_UIType::getDisplayValue( $fieldvalue );
    } else if ($module == 'PBXManager' && $fieldType == 'string') {
        if ($fieldInfo['fieldname'] == 'customertype') {
            $moduleName = $fieldvalue;
        } else {
            $moduleName = $module;
        }
        $fieldvalue = vtranslate($fieldvalue, $moduleName);
	} elseif ( $fieldInfo['uitype'] == 780 && substr( $fieldInfo['fieldname'], 0, 10 ) === 'cf_pcf_cul' ) {
		$fieldvalue = trim( $fieldvalue );
		$userIds    = explode( ',', $fieldvalue );
		$users      = array();
		foreach ( $userIds as $id ) {
			if ( $id ) {
                if (class_exists('ParsVT_Constant_Model')) {
                    $users[] = ParsVT_Constant_Model::getUserFullName(intval($id)) != '' ? ParsVT_Constant_Model::getUserFullName(intval($id)) : ParsVT_Constant_Model::getGroupFullName(intval($id));
                } else {
                    $users[] = getUserFullName( intval( $id ) );
                }
			}
		}
		$fieldvalue = implode(',', $users);
	}
	...
	} elseif ( ($fld_type == "datetime" || $fld_type == "customdatetime") && empty( $fieldType ) ) {//by mahdi
		$date = new DateTimeField($fieldvalue);
		$fieldvalue = $date->getDisplayDateTimeValue();
	}
    ...
    foreach($advFilterList as $k => $columnConditions) {
        foreach($columnConditions['columns'] as $j => $columnCondition) {
            ...
            if(($columnInfo[4] == 'D' || ($columnInfo[4] == 'T' && $columnInfo[1] != 'time_start' && $columnInfo[1] != 'time_end') ||
                ...
                $val = Array();
                for($i=0; $i<ParsVT_Module_Model::php7_count($tempVal); $i++) {
                    if(trim($tempVal[$i]) != '') {
...
function getReportSearchCondition($searchParams, $filterId) {
	if (!empty($searchParams)) {
		...
		foreach ($searchParams as $i => $condition) {
			...
			if ($i < (ParsVT_Module_Model::php7_count($searchParams) - 1)) {
				$conditionQuery .= ' AND ';
			}
		}
		$conditionQuery .= " ) ";
