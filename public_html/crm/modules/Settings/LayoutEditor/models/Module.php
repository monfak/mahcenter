...
/**PVTPATCHER-F0EB96B36B7DD4AFDACAFE09B4E469C4-START-advcf**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:23 **/
if (isset($params['fieldTableList']) && !empty($params['fieldTableList'])) {
            if (!Vtiger_Utils::CheckTable($params['fieldTableList'])) {
                throw new Exception(sprintf(vtranslate('Table %s does not exist in the database', 'ParsVT'),[$params['fieldTableList']]), 513);
            }
        }
        if (isset($params['fieldName']) && !empty($params['fieldName'])) {
            $params['fieldName'] = strtolower($params['fieldName']);
            if (!preg_match('/([a-z0-9\_]+)*$/', $params['fieldName'])) {
                throw new Exception(sprintf(vtranslate('invalid value detected: %s', 'ParsVT'),[$params['fieldName']]), 513);
            }
            ...
            $result = $db->pquery($query, array($tabId,$params['fieldName']));
            $checker =  ($db->num_rows($result) > 0 ) ? true : false;
            if ($checker) {
                throw new Exception(vtranslate('LBL_DUPLICATE_FIELD_EXISTS', 'Settings::LayoutEditor'), 513);
            }
        }
        if (isset($params['fieldTableList']) && !empty($params['fieldTableList']) && Vtiger_Utils::CheckTable($params['fieldTableList'])) {
            $tableName = $params['fieldTableList'];
        }
        if (isset($params['fieldName']) && !empty($params['fieldName'])) {
            $columnName = $params['fieldName'];
        }
/**PVTPATCHER-F0EB96B36B7DD4AFDACAFE09B4E469C4-FINISH**/
