...
/**PVTPATCHER-D0E4CBBFA7FE180F3499E39AFAC49FFF-START-advcf**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:23 **/
$fileName = urlencode($fileName);
/**PVTPATCHER-D0E4CBBFA7FE180F3499E39AFAC49FFF-FINISH**/
...
/**PVTPATCHER-90C08C3EAD69125BA42FEF41E4917E65-START-advcf**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:23 **/
} elseif($uitype == 21 && substr( $fieldName, 0, 10 ) === 'cf_pcf_arf') {//repeater field
                        $value = $this->repeaterView($value);
        			} elseif($uitype == 780 && substr( $fieldName, 0, 10 ) === 'cf_pcf_cul') {//user list field
                        $value = $this->listUserView($value);
        			} elseif($uitype == 777 && substr( $fieldName, 0, 10 ) === 'cf_pcf_icf') {//city field
        			    $model = Vtiger_Module_Model::getInstance($moduleName);
                        $fieldInstance = Vtiger_Field_Model::getInstance($fieldName, $model);
                        $value = $fieldInstance->getDisplayValue($value);
        			} elseif($uitype == 788 && substr( $fieldName, 0, 10 ) === 'cf_pcf_csf') {//city field
        			    $model = Vtiger_Module_Model::getInstance($moduleName);
                        $fieldInstance = Vtiger_Field_Model::getInstance($fieldName, $model);
                        $value = $fieldInstance->getDisplayValue($value);
        			} elseif($uitype == 1 && substr( $fieldName, 0, 10 ) === 'cf_pcf_ulf') {//Upload field
                        $value = $this->multiFileUploadView($value);
        			} elseif($uitype == 19 && substr( $fieldName, 0, 10 ) === 'cf_pcf_muf') {//Upload multi field
                        $value = $this->multiFileUploadView($value);
        			} elseif($uitype == 35 && substr( $fieldName, 0, 10 ) === 'cf_pcf_vgf') {//Grid field
        			    $length = strlen($value);
        			    if($value && substr( $value, $length-1, 1 ) === '_')$value =substr( $value,0, $length-1 );
/**PVTPATCHER-90C08C3EAD69125BA42FEF41E4917E65-FINISH**/
...
/**PVTPATCHER-75ACB0B4506EC5753D200F621DA73776-START-advcf**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:23 **/
public function repeaterView($str){
                $str = trim($str);
                ...
                if ($lastChar == '_') {
                    ...
                    foreach ($json_array as $value){
                        if(isset($value['repeater']))$repeater .= $value['repeater']."\n";
                    }
                    return $repeater;
                } else {
                    return $str;
        	...
        	public function multiFileUploadView($str){
        	    if(!$str)return '';
                global $site_URL;
                $url = 'index.php?module=ParsVT&action=ActionAjax&mode=downloadFile&file=';
                $str = trim($str);
                $files = explode('||' ,$str);
                $link = '';
                foreach ($files as $file){
                    if($file){
                        $link .= $site_URL.$url.$file."\n";
                    }
                }
                return $link;
            }

        	public function listUserView($str){
        	    if(!$str)return '';
                $str = trim($str);
                $userIds = explode(',' ,$str);
                $users = array();
...
/** REPLACED-75ACB0B4506EC5753D200F621DA73776// public function moduleFieldInstances($moduleName) {**/
/**PVTPATCHER-75ACB0B4506EC5753D200F621DA73776-FINISH**/
