...
class Vtiger_Module_Model extends Vtiger_Module {
	...
	public function getOwnerWhereConditionForDashBoards ($owner) {
		...
		if(!empty($owner) && $currentUserModel->isAdminUser()) {//If admin user, then allow users data
			$ownerSql =  '( smownerid = '. $owner .' or  smownerid IN (select groupid from vtiger_users2group where userid = '.$owner.'))';
			$params[] = $owner;
			...
			if($sharingAccessModel->isPrivate()) {
				...
				if(in_array($owner, $subordinateUsers)) {
					...
					$ownerSql = '( smownerid = '. $currentUserModel->getId() .' or  smownerid IN (select groupid from vtiger_users2group where userid = '.$currentUserModel->getId().'))';
				}
			} else {
				$ownerSql = '( smownerid = '. $owner .' or  smownerid IN (select groupid from vtiger_users2group where userid = '.$owner.'))';
			}
...
/**PVTPATCHER-F5F583943465861AFD31DE8A37F051FF-START-ws**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:30:38 **/
global $transferRecordsOwnership;
            $transferRecordsOwnership = true;
/**PVTPATCHER-F5F583943465861AFD31DE8A37F051FF-FINISH**/
