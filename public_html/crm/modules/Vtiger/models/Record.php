...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	public function transferRelationInfoOfRecords($recordIds = array()) {
		if ($recordIds) {
			$moduleName = $this->getModuleName();
			$focus = CRMEntity::getInstance($moduleName);
/**PVTPATCHER-FB83932FB932DF192A6920C85C1745AF-START-duplicates**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:36:04 **/
if (method_exists('ParsVT_Duplicates_Model', 'transferRelatedRecords')) {
                ParsVT_Duplicates_Model::transferRelatedRecords($moduleName, $recordIds, $this->getId());
            } else
/**PVTPATCHER-FB83932FB932DF192A6920C85C1745AF-FINISH**/
...
/**PVTPATCHER-34C8A293EDAA4BE255AD6148A51ED644-START-chars**/
/** Don't remove the Start and Finish Markup! Modified: 2025-01-09 09:29:42 **/
if (!file_exists($filePath.$savedFile)) {
                    $newfiles = glob($filePath.$fileDetails['attachmentsid'].'_*');
                    if (!empty($newfiles)) {
                        $savedFile = str_replace($filePath,"",$newfiles[0]);
                    }
                }
/**PVTPATCHER-34C8A293EDAA4BE255AD6148A51ED644-FINISH**/
