            ...
            <div class="box">
                ...
                <div class="box-body">
                    <div class="nav-tabs-custom no-shadow">
                        ...
                        <form method="post" action="{{ route('admin.discounts.update',$discount->id) }}" role="form" class="form-horizontal" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <div class="tab-content">
                                <div id="tab_general" class="tab-pane active">
                                    <div class="form-group">
                                        <label for="title" class="col-md-2 control-label">عنوان تخفیف *</label>
                                        <div class="col-md-10">
                                            <input type="text" name="title" readonly id="title" class="form-control" value="{{ old('title',$discount->title) }}">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="value" class="col-md-2 control-label">درصد تخفیف *</label>
                                        <div class="col-md-10">
                                            <input type="number" min="1" max="99" name="value" id="value" class="form-control" value="{{ old('value',$discount->value) }}">
                                        </div>
                                    </div>
...
{{--                                    </div>--}}
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group date">
                                                ...
                                                <input type="text" id="start_date_datepicker" name="start_date_datepicker" class="form-control pull-right" autocomplete="off" value="{{ date('Y-m-d H:i', old('start_date',  $discount->start_date)) }}">
                                                <input type="hidden" id="start_date" name="start_date" class="form-control pull-right" value="{{ old('start_date',$discount->start_date) }}">
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group date">
                                                ...
                                                <input type="text" id="end_date_datepicker" name="end_date_datepicker" class="form-control pull-right" autocomplete="off" value="{{ date('Y-m-d H:i', old('end_date',  $discount->end_date)) }}">
                                                <input type="hidden" id="end_date" name="end_date" class="form-control pull-right" value="{{ old('end_date',$discount->end_date) }}">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-10">
                                            <textarea name="content" id="content" class="form-control tinymce" readonly rows="5">{{ old('content',$discount->content) }}</textarea>
                                        </div>
                                    </div>
                                       <div class="form-group user_target customer_target {{ old('users', 0) == 0 ? '' : 'hide' }}">
                                        ...
                                        <div class="col-md-10">
                                            <select name="customer_id[]" class="form-control customers" multiple="multiple" readonly>
                                                @foreach($users as $user)
                                                    <option value="{{ $user->id }}" {{ (in_array($user->id, old('customer_id',$select_user) ?? []) ? ' selected' : '') }}>{{ $user->first_name. ' '.$user->last_name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="raw">
                                        <!--<div class="col-md-8 col-md-offset-2"><small>اگر کاربر خاصی مد نظرتان نیست کاربری انتخاب نشود.</small></div>-->
                                    </div>
                                </div>
                            </div>
