...
@section('title', $site_settings['festival_title'])
@section('seo')
...
@endsection
@section('content')
    @if($festivalBanners->status)
    <section class="container-fluid banner-section pt-3 mb-3 pb-3 p-xs-0">
      <div class="festival-container">
        <div class="festival-hero-wrapper">
            @foreach($festivalBanners->items as $item)
                @if($loop->first)
                    <div class="festivalhero-main">
                        <div class="festival-hero-box">
                            <a href="{{ $item->url }}">
                ...
                @endif
                @if($loop->index === 1)
                <div class="{{ $loop->first ? 'festivalhero-main' : 'festival-hero-side' }}">
                @endif
                <div class="festival-hero-box">
                    <a href="{{ $item->url }}">
                        <img src="{{ asset($item->image) }}" class="img-fluid" alt="{{ $item->title }}">
    ...
    <div class="container container-pro">
        <div class="row mt-4 mb-4">
            <div class="col-12 p-0">
                 <h1 class="title-section filter-caption form-control">{{ $site_settings['festival_heading'] }}</h1>
            </div>
        </div>
        <div class="row mt-4 mb-4">
                            ...
                            <div class="product-thumb list-view product-main-categori">
                                <div class="image">
                                    <a href="{{ url('products/' . $product->slug) }}">
                                        <img src="{{ asset(image_resize($product->image, ['width' => 228, 'height' => 228])) }}" alt="{{ $product->alt }}" title="{{ $product->name }}" class="img-fluid">
                                    </a>
                                </div>
                                <div class="d-block">
                                    <div class="img-brand">
